...
if [ "${1:-}" = "--help" ]; then
  ...
  ${T}tmux binding${R}  ${D}fmux owns one file and borrows one line of your ~/.tmux.conf${R}
    ...
    ${D}a prefix-less key takes that key from everything in the pane — vim, line editing, fzf${R}
  ...
  ${T}status bar${R}  ${D}the snippet puts the tally at the front of status-right — yours survives${R}
    ⏸n ✻n — sessions awaiting you / working right now    ✓name — done while away, clears on visit or ${hunseen} min
