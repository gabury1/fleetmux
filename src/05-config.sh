...
fmux_conf_default() {
    case "${1:-}" in
        ...
        key_summon)      printf 'F' ;;
        # The no-prefix summon key. This used to default to empty, so that installing the config
        # file alone never took a key from anybody — a no-prefix binding grabs that key from every
        # app in the pane (vim, shell, fzf), and that felt like something a person should say yes
        # to first.
        #   Measured against what actually happened: a teammate installed it, found the one key
        #   the tool is about did nothing, and had to go set it by hand (2026-08-11). The caution
        #   cost every new user the feature, to protect a key almost nobody had bound.
        #   So the default is S-Up. Why that key and not another: Shift+arrow is the only
        #   no-prefix single keystroke that reaches tmux on macOS, Linux and Windows Terminal
        #   alike — Ctrl+arrow is taken by Mission Control, Alt+arrow by Windows Terminal, and
        #   Ctrl+letter by shell line editing, vim and fzf. install.sh writes this out at length.
        #   Taking it back is one line, and the popup's help screen says which line:
        #     fmux config unset key_summon_fast
        #   The snippet emits `unbind -n` for the old key when you do, so it goes back to whatever
        #   it did before.
        key_summon_fast) printf 'S-Up' ;;
