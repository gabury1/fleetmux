...
#
#   폭 계산이 필요한 이유 — 접힘이 아니라 **누가 자르느냐**다.
#     90-main.sh 의 --preview-window 에는 wrap 이 없고 fzf 의 기본은 nowrap 이다. 그래서
#     폭을 넘는 줄은 접히지 않는다(줄 수 회계는 안 깨진다) — fzf 가 **말없이** 잘라낸다.
#     프리뷰 맨 위 그 줄은 "내가 뭘 시켰더라"의 답이라, 뒤가 더 있다는 표시 없이 끊기면
#     남은 부분을 지시의 끝으로 읽는다. 그래서 우리가 먼저 잘라 … 를 붙인다.
#     한글은 한 글자가 두 칸이라 바이트로도 글자 수로도 못 잰다 → UTF-8 을 직접 해독한다.
#
...
TT_LASTP_VIEW_AWK='
    ...
    #     스톱인데 우리는 앞머리 "❯ " 를 붙이므로 스톱이 어디 서는지 알 수 없다. 폭 1 로
    ...
    #   · 홀로 선 연속 바이트(0x80–0xBF)도 버린다. 폭이 정의되지 않는 깨진 조각이고,
    #     0x9B 한 바이트는 8비트 터미널에서 CSI 로 읽힌다.
