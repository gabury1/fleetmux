...
# 대기 화면 판정 — **⏸ 박제 해제의 유일한 증인**(80-view.sh 의 waiting 분기).
# 훅이 60초 넘게 조용한데 화면에도 프롬프트가 없으면 "취소된 승인"으로 보고 ⏸ 를 지운다.
# 즉 여기서 못 잡는 프롬프트 UI 는 **목록에서 ⏸ 가 사라진다**(상태바엔 남아 둘이 어긋난다 — 실측 버그).
#
# 그래서 이 패턴의 기울기는 WORKING_PAT 과 반대다: **넓게 잡는 쪽이 안전하다.**
#   못 잡으면 → 기다리는 세션이 목록에서 익명이 된다(사람이 영영 안 돌아온다)
#   넓게 잡으면 → 취소된 승인이 조금 늦게 꺼진다(다음 훅 이벤트가 어차피 덮는다)
#
# 각 대안이 어느 화면을 받는지:
#   Do you want to        claude 도구 승인 ("Do you want to proceed?" · "…make this edit to X?")
#   Would you like to     claude 플랜 승인 ("Would you like to proceed?") — 옛 패턴은 'to run' 만
#                         받아서 플랜 승인을 통째로 놓쳤다
#   Enter to select       AskUserQuestion·선택 메뉴의 꼬리
#                         ("Enter to select · Tab/Arrow keys to navigate · Esc to cancel")
#   Esc to cancel         같은 꼬리의 뒷도막. 메뉴가 떠 있으면 둘 중 하나는 남는다
#   Press enter to confirm / Yes, proceed    codex 승인
#
# ⚠️ 'esc to interrupt' 를 여기 넣지 마라 — 그건 **작업중**의 표지고(WORKING_PAT ①), 넣으면
#    돌고 있는 세션이 대기로 박제된다.
