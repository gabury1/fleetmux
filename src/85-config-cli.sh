...
tt_conf_wired_scan() {
    ...
    TT_CONF_WIRED_SCANNED=1                      # don't retry even on failure — pin it to a single fork
    local out=''
    [ -r "$SELF" ] || return 0
    out=$(awk "$TT_CONF_WIRED_AWK" "$SELF" 2>/dev/null) || out=''
    [ -n "$out" ] || return 0
    TT_CONF_WIRED_LIST=" $out"                   # includes leading/trailing space — case-glob sees exactly one word
    return 0
}
tt_conf_wired() {
    tt_conf_wired_scan
...
tt_conf_validate() {
    ...
    case "$k" in
        ...
        key_*)
            ...
            tt_conf_is_fzf_key "$v" || { echo "not a key name fzf knows: $v (e.g. ctrl-n, alt-x, f2)" >&2; return 1; }
...
#   (recommendation N4). Validation reuses the existing tt_conf_validate and just swallows the reason.
tt_conf_lie() {
    tt_conf_validate "${1:-}" "${2:-}" 2>/dev/null && return 0
    case "${1:-}" in
        rc|snapshot|snapshot_on_exit|boot_restore)  printf '← invalid value · falls back to off' ;;
        recent_hours|unseen_minutes|accent|log_max) printf '← invalid value · falls back to default %s' "$(tt_conf_default "$1")" ;;
...
tt_conf_write() {
    ...
    tmp="$TT_CONF.tmp.$$"
    : > "$tmp" || { echo "cannot write config file: $TT_CONF" >&2; return 1; }
    if [ -f "$TT_CONF" ]; then
...
if [ "${1:-}" = "config" ]; then
    # Each subcommand below walks TT_CONF_KEYS and calls tt_conf_get/tt_conf_source many times
    # wrapped in $(...) — each call is a subshell, so unless tt_conf_load is fired here first as a
    # bare statement, every subshell re-inherits TT_CONF_LOADED=0 and re-parses the config file once
        ...
        ''|list)
            ...
            for k in $TT_CONF_KEYS; do
                ...
                if tt_conf_wired "$k"; then
                    ...
                    unwired=1
                    printf '%-18s %-14s %-9s%s%s\n' "$k" "$v" "$(tt_conf_source "$k")" '← not wired' "$lie"
                fi
            done
