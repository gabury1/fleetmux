...
# Run it in a subshell so the deliberate failure does not pollute this file's real FMUX_FAIL.
_neg=$( ( FMUX_FAIL=0; assert_eq "a" "b" "deliberate mismatch"; printf 'FMUX_FAIL=%s' "$FMUX_FAIL" ) )
case "$_neg" in
    *FAIL*FMUX_FAIL=1*) printf '  ok   assert_eq catches a mismatch as a failure\n' ;;
    *) printf '  FAIL assert_eq did not catch a mismatch as a failure: %s\n' "$_neg"; FMUX_FAIL=$((FMUX_FAIL+1)) ;;
esac

_neg=$( ( FMUX_FAIL=0; assert_contains "hello world" "zzz" "deliberate mismatch"; printf 'FMUX_FAIL=%s' "$FMUX_FAIL" ) )
case "$_neg" in
    *FAIL*FMUX_FAIL=1*) printf '  ok   assert_contains catches a missing substring as a failure\n' ;;
...
# and attaches to the real server. Confirm fmux_test_sandbox unset it.
if [ -z "${TMUX:-}" ]; then
    printf '  ok   TMUX is isolated (unset)\n'
else
    printf '  FAIL TMUX is still set: %s\n' "$TMUX"; FMUX_FAIL=$((FMUX_FAIL+1))
fi

# ── Sealed fake tmux ─────────────────────────────────────────────────────
# Socket isolation only decides "which server". The real tmux binary running at all is itself a
# hazard, so fmux_test_sandbox puts a fake one first on PATH. Measure here that the seal actually held.
_which_tmux=$(command -v tmux 2>/dev/null || true)
case "$_which_tmux" in
    "$FMUX_TMUX_STUB_DIR/tmux") printf '  ok   the tmux on PATH is the sealed fake: %s\n' "$_which_tmux" ;;
    *) printf '  FAIL the tmux on PATH is not the fake: %s\n' "$_which_tmux"; FMUX_FAIL=$((FMUX_FAIL+1)) ;;
esac
...
_refused=$(wc -l < "$FMUX_TMUX_STUB_REFUSED" | tr -d ' ')
assert_eq "$_refused" "12" "all 12 refused subcommands landed in the log"
assert_contains "$(cat "$FMUX_TMUX_STUB_REFUSED")" "kill-server" "the refusal log records the arguments too"
assert_contains "$(cat "$FMUX_TMUX_STUB_LOG")" "kill-server -t nope" "it also lands in the full call log"
...
_neg=$( ( FMUX_FAIL=0; assert_no_tmux_mutation "deliberate"; printf 'FMUX_FAIL=%s' "$FMUX_FAIL" ) )
case "$_neg" in
    *FAIL*FMUX_FAIL=1*) printf '  ok   assert_no_tmux_mutation catches the refusal log as a failure\n' ;;
...
# fmux_test_sandbox must set FMUXBIN (the Interfaces contract in the briefing) — this must hold even
# when this file is run standalone (`bash test/t-00-harness.sh`) without going through run.sh.
case "${FMUXBIN:-}" in
    */bin/fmux) printf '  ok   FMUXBIN is set: %s\n' "$FMUXBIN" ;;
    *) printf '  FAIL FMUXBIN is not set: %s\n' "${FMUXBIN:-}"; FMUX_FAIL=$((FMUX_FAIL+1)) ;;
esac

# A second call within the same process must be refused (so the first FMUXROOT does not leak).
if ( fmux_test_sandbox ) >/dev/null 2>&1; then
    printf '  FAIL a repeat call to fmux_test_sandbox was not refused\n'; FMUX_FAIL=$((FMUX_FAIL+1))
else
    printf '  ok   a repeat call to fmux_test_sandbox is refused\n'
fi
