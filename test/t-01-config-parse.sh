...
assert_eq "$("$FMUXBIN" config get recent_hours)"    "12"           "numeric values are read too"
assert_eq "$("$FMUXBIN" config get key_summon_fast)" "C-Left M-b"   "a value with spaces (a list) is read"
...
accent=200
EOF
assert_eq "$("$FMUXBIN" config get rc)"     "off"  "an earlier value survives even with a broken line present"
assert_eq "$("$FMUXBIN" config get accent)" "200"  "a value after a broken line survives too"
assert_contains "$("$FMUXBIN" config get rc 2>&1 >/dev/null)" "ignoring" "it warns that a line was ignored"
...
rm -rf $HOME
EOF
_warn=$("$FMUXBIN" config get rc 2>&1 >/dev/null)
assert_contains "$_warn" "not key=value shape" "a line with no '=' warns too"
assert_eq "$("$FMUXBIN" config get rc)" "off" "other values survive even with a line missing '='"
...
#    FMUX_CONF_KEYS and calls `$(fmux_conf_get "$k")`/`$(fmux_conf_source "$k")` in a fresh subshell per
#    key — if each subshell inherited a fresh FMUX_CONF_LOADED=0, it would re-parse the file and
...
unknown_key=1
EOF
_warn=$("$FMUXBIN" config 2>&1 >/dev/null)
...
#    If fmux_conf_load only checked with `-f`, it would pass the existence check and then the
#    redirect on `done < "$FMUX_CONF"` would fail, and 00-header's set -e would catch that and
...
if [ "$(id -u)" = 0 ]; then
    ...
    printf 'rc=off\n' > "$CONF"
    chmod 000 "$CONF"
    assert_rc 0 "$FMUXBIN" config get rc
    assert_eq "$("$FMUXBIN" config get rc 2>/dev/null)" "on" \
        "★an unreadable config file folds back to the default (on, not the off it would have gotten by parsing)"
    assert_eq "$("$FMUXBIN" config source rc 2>/dev/null)" "default" "source honestly reports default too"
    # The listing / CLI entry point must survive too — if this dies, the whole config screen never opens
    assert_rc 0 "$FMUXBIN" config
    chmod 644 "$CONF"
    assert_eq "$("$FMUXBIN" config get rc)" "off" "restoring permissions reads the file value again (the guard does not just ignore the file forever)"
    : > "$CONF"
fi
