#!/usr/bin/env bash
# CPU-delta-based working verdict — tt_cpu_read / tt_cpu_cs_ps / tt_cpu_sample / tt_cpu_busy.
#
...
#    /proc is faked by injecting a directory through TT_PROC, and the macOS fallback is reproduced by
...
# ── ① tt_cpu_read — /proc parsing ────────────────────────────────────────
fake_proc 4242 1000 234
TT_CPU_CS=""; TT_CPU_Q=""
assert_eq "$(rc_of tt_cpu_read 4242)" "0" 'tt_cpu_read: reads the fake /proc'
tt_cpu_read 4242
assert_eq "$TT_CPU_CS" "1234" 'adds utime+stime (never adds cutime/cstime — reap spikes)'
assert_eq "$TT_CPU_Q" "1" '/proc resolution is 1cs (USER_HZ=100 fixed)'
assert_eq "$(rc_of tt_cpu_read 0)" "1" 'pid 0 cannot be read'
assert_eq "$(rc_of tt_cpu_read abc)" "1" 'cannot be read if pid is not numeric'
printf 'garbage without parens\n' > "$PROC/4242/stat"
assert_eq "$(rc_of tt_cpu_read 4242)" "1" 'cannot be read if /proc stat is not the right format (does not leave a stale value behind)'
fake_proc 4242 1000 234

# ── ② tt_cpu_cs_ps — the macOS/BSD fallback parser ──────────────────────────
...
assert_eq "$TT_CPU_Q" "1" 'resolution is 1cs when there is a decimal point — the normal Mac path'
PS_OUT="00:07:48"; PS_RC=0; tt_cpu_cs_ps 1 >/dev/null 2>&1 || true
assert_eq "$TT_CPU_Q" "100" 'resolution is 100cs when there is no decimal point → the caller extends the minimum window to 20 seconds'
PS_RC=1
assert_eq "$(rc_of tt_cpu_cs_ps 1)" "1" 'cannot be read if ps fails'
PS_OUT=""; PS_RC=0
assert_eq "$(rc_of tt_cpu_cs_ps 1)" "1" 'cannot be read if ps gives an empty line (not fooled by a pipe rc of 0)'
PS_RC=1

# ── ③ tt_cpu_busy — the verdict ─────────────────────────────────────────────
...
assert_eq "$(rc_of tt_cpu_busy '' 4242 "$NOW")" "2" 'cannot be judged if sid is empty'
assert_eq "$(rc_of tt_cpu_busy s1 0 "$NOW")" "2" 'cannot be judged if pid is unknown (hook failed to climb the parent chain)'
...
assert_eq "$(rc_of tt_cpu_busy s1 4242 "$NOW")" "0" 'above threshold (8.0 cs/s): working'
snap 4242 $((NOW - 5)) 1214 0 0                          # Δ20 / 5s = 4.0 cs/s
assert_eq "$(rc_of tt_cpu_busy s1 4242 "$NOW")" "1" 'below threshold (4.0 cs/s): not working — the idle max range measured in practice'
snap 4242 $((NOW - 5)) 1204 0 0                          # Δ30 / 5s = exactly 6.0
assert_eq "$(rc_of tt_cpu_busy s1 4242 "$NOW")" "0" 'the boundary value 6.0 cs/s is inclusive (>=)'
snap 4242 $((NOW - 5)) $((1204 + 1)) 0 0                 # Δ29 → 5.8
assert_eq "$(rc_of tt_cpu_busy s1 4242 "$NOW")" "1" 'just below the boundary is excluded'
...
assert_eq "$(rc_of tt_cpu_busy s1 4242 "$NOW")" "0" 'judges when the window is exactly MINWIN (3 seconds)'
...
assert_eq "$(rc_of tt_cpu_busy s1 4242 "$NOW")" "2" 'cannot be judged if the window is negative (NTP jump, suspend/resume)'
...
assert_eq "$(rc_of tt_cpu_busy s1 4242 "$NOW")" "1" 'not working if even the previous sample is below threshold'
snap 4242 $((NOW - 1)) 1230 0 0                          # no previous sample
assert_eq "$(rc_of tt_cpu_busy s1 4242 "$NOW")" "2" 'cannot be judged if there is no previous sample and the newest one is too young'

# Self-invalidation — even if the file rots, it does not misjudge using a stale value
snap 9999 $((NOW - 5)) 0 0 0
assert_eq "$(rc_of tt_cpu_busy s1 4242 "$NOW")" "2" '★pid mismatch (number reused after a reboot/restart) → the snapshot is discarded'
snap 4242 $((NOW - 5)) 99999 0 0                         # the counter went backward
assert_eq "$(rc_of tt_cpu_busy s1 4242 "$NOW")" "2" 'cannot be judged if the delta is negative (counter reset)'
snap 4242 abc 1194 0 0
assert_eq "$(rc_of tt_cpu_busy s1 4242 "$NOW")" "2" 'a rotten line cannot be judged (an arithmetic error does not kill the list)'
printf '' > "$STATE/cpu-s1"
assert_eq "$(rc_of tt_cpu_busy s1 4242 "$NOW")" "2" 'an empty file also cannot be judged'
...
assert_eq "$(rc_of tt_cpu_busy s1 4242 "$NOW")" "1" 'judges even at 1-second resolution once the window exceeds 20 seconds'
snap 4242 $((NOW - 25)) 1000 0 0                         # Δ200 / 25s = 8.0 cs/s
assert_eq "$(rc_of tt_cpu_busy s1 4242 "$NOW")" "0" 'above threshold is working even at coarse resolution'
PS_RC=1
fake_proc 4242 1000 234

# ── ④ tt_cpu_sample — rotation ────────────────────────────────────────────
rm -f "$STATE/cpu-s1"
...
#   sampler returns immediately every time → the file never updates again, and tt_cpu_busy keeps
...
tt_cpu_sample s1 4242 $((NOW + 5))                       # ROTATE (3s) has elapsed → rotates
...
assert_eq "$(rc_of tt_cpu_busy s1 555 $((NOW + 5)))" "0" 'sample→verdict round trip: leaves a sample and the delta stands on the next call'
