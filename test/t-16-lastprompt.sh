...
#      where tt_jv's regex used to break are all pinned down here.
...
case "${1:-}" in
    ls)
        # Split the call site by format string: 5 fields = --list, 2 fields = tt_sweep_hooks
        case "${3:-}" in
...
# ★The spot where tt_jv's regex used to break completely — a quote inside the value.
#   Measured in practice: for {"prompt":"the \"dependencies\" section of the README"}, tt_jv
...
#   content is placed under ~/.cache/tt, and ~/.claude/projects, which holds the same kind of
...
#   shell). What gets pulled in is bin/fmux verbatim — from TT_LASTP_MAX through the closing
#   brace of tt_last_prompt_save is a pure definition with no side effects.
eval "$(sed -n '/^TT_LASTP_MAX=/,/^}$/p' "$TTBIN")"
assert_eq "$(command -v tt_last_prompt_save)" "tt_last_prompt_save" "the save function was pulled into this shell"
rm -f "$LF"
frag="$STATE/.last-9.$$"
printf 'x' > "$frag"; chmod 666 "$frag"
oldmask=$(umask); umask 000
tt_last_prompt_save '$9' "$(ups 'write over the leftover fragment')" 1700000000
umask "$oldmask"
...
assert_eq "$("$TTBIN" --preview zz 2>/dev/null || true)" "$PANE" "★it stays the same even without a session id argument"
...
assert_contains "$(cat "$TTBIN")" "--preview {1} {3}" "★fzf passes that field 3 to the preview"
assert_contains "$(cat "$TTBIN")" "--with-nth=2" "only field 2 is displayed — the id does not leak onto the screen"
