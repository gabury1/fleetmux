...
assert_eq "$("$TTBIN" config path)" "$CONF" "config path gives the path"
...
assert_eq "$("$TTBIN" config get rc)" "off" "a rejected set does not change the file"
...
assert_contains "$("$TTBIN" config set key_new ctrl-e 2>&1)" "key_rename" "names the key it conflicts with"
...
assert_eq "$("$TTBIN" config get accent)" "73" "unset reverts to the default value"
...
# ⑨ set's key-conflict check (the `for other in $TT_CONF_KEYS ... $(tt_conf_get "$other")` loop
#    inside tt_conf_validate) also forks a subshell per key — a malformed line here could also have
#    repeated its warning once per key. The single bare tt_conf_load at the top of the `config`
...
: > "$CONF"
assert_eq "$("$TTBIN" config list 2>/dev/null | grep -c 'invalid value' || true)" "0" \
